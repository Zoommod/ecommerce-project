...
class Produto:
    ...
    def mostrar_produtos(self):
        print(f"\n{CorTexto.VERDE}Nome: {CorTexto.BRANCO}{self.get_nome()}")
        print(f"{CorTexto.VERDE}Código: {CorTexto.BRANCO}{self.get_codigo()}")
        print(f"{CorTexto.VERDE}Descrição: {CorTexto.BRANCO}{self.get_descricao()}")
        print(f"{CorTexto.VERDE}Valor: {CorTexto.BRANCO}R${self.get_valor()}\n")
        print("=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*" * 2)
...
produto4 = Produto('SSD 1 TB Kingston NV2, M.2 2280 PCIe', '0004',
                   '{CorTexto.BRANCO}O NV2 PCIe 4.0 NVMe SSD da Kingston é uma solução substancial de armazenamento de última geração alimentada por um controlador Gen 4x4 NVMe. O NV2 oferece velocidades de leitura/gravação de até 3.500/2.100 MB/s\ncom menores requisitos de energia e menor aquecimento para ajudar a otimizar o desempenho do seu sistema e agregar valor sem sacrifícios.',
                   294)
